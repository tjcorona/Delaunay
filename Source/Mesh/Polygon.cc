...
namespace
{
  ...
  typedef Delaunay::Shape::Polygon::Vertex Vertex;
  ...
  bool OnLineSegment(const Point& p, const Point& p1, const Point& p2)
  {
  ...
  bool IsInternal(const Vertex& v,const Point& w)
  {
    // Is line segment (v,w) within the angle formed at vertex v?
    const Vertex& v0 = v.Previous();
    const Vertex& v1 = v.Next();
  ...
  bool IsADiagonal(const Vertex& v1, const Vertex& v2)
  {
...
Polygon::Polygon(const std::vector<Point>& vertices) : Vertices(V)
{
...
Polygon::Polygon(const Polygon& other) : Vertices(V)
{
