		   ...
		   unsigned nSteps)
{
  ...
  for (unsigned i=0;i<nVertices;i++)
  {
    ...
    direction.push_back((Random::GetInstance().Uniform(1000)/1000.)*(2.*M_PI));
    velocity.push_back((Random::GetInstance().Uniform(1000)/1000.)*.25);
  }
  ...
  for (unsigned step=0;step<nSteps;step++)
  {
	...
	vertices[i].y = c.y;
      ...
      {
	direction[i] = (Random::GetInstance().Uniform(1000)/1000.)*(2.*M_PI);
      }
    }
  }
}
...
int main(int argc,char** argv)
{
  ...
  if (testType == Random)
    testType = static_cast<PolygonType>(Random::GetInstance().Uniform(3));
  ...
  DTCVCanvas canvas(bounds[0],bounds[1],bounds[2],bounds[3]);

  const unsigned nVertices = 3 + Random::GetInstance().Uniform(100);
  std::vector<DTPoint> vertices;
  ...
  if (testType == Regular)
  {
    ...
    for (unsigned i=0;i<nVertices;i++)
    {
      vertices.push_back(DTPoint(5. + 4.*cos(theta), 5. + 4.*sin(theta)));
  ...
  else if (testType == StarConvex)
    {
      ...
      {
	sharedTheta = false;
	double min_radius =.1+3.9*(Random::GetInstance().Uniform(10000)/10000.);
	double max_radius = 5. - min_radius;
	for (unsigned i=0;i<nVertices;i++)
	{
	  radii[i] = (min_radius +
		      max_radius*(Random::GetInstance().Uniform(10000)/10000.));
	  thetas[i] = Random::GetInstance().Uniform(360)/180.*M_PI;
	}
	...
	for (unsigned i=0;i<nVertices;i++)
	{
      ...
      for (unsigned i=0;i<nVertices;i++)
	vertices.push_back(DTPoint(5. + radii[i]*cos(thetas[i]),
				   5. + radii[i]*sin(thetas[i])));
    }
  else if (testType == RandomEvolve)
  {
    std::vector<Coord> verts;
    RandomPolygon(nVertices,verts,bounds,Random::GetInstance().Uniform(50));
    for (unsigned i=0;i<nVertices;i++)
      vertices.push_back(DTPoint(verts[i].x,verts[i].y));
  }

  // for (unsigned i=0;i<vertices.size();i++)
  //   std::cout<<"vertices.push_back(DTPoint("<<vertices[i].x<<","<<vertices[i].y<<"));"<<std::endl;
  // std::cout<<""<<std::endl;

  // create a triangle from these three verties
  DTPolygon polygon(vertices);

  std::vector<DTTriangle*> triangles;
  ...
  {
    canvas.DrawTriangle(*triangles[i],Green);
    canvas.DrawPoint((triangles[i]->A + triangles[i]->B + triangles[i]->C)/3.,
		     Green);
  }

  canvas.DrawPolygon(polygon,Red);

  for (unsigned i=0;i<polygon.Vertices.size();i++)
    canvas.DrawPoint(vertices[i],DTColor(i/(polygon.Vertices.size()-1.),
					 DTColor::BlueToRed));
